...
function pathm = pivdirs (pathin, quality, pathout)
    if(pathin(end)~='/') pathin(end+1) = '/'; end
    % pathm = 'mat/';
    % 
    b = dir([pathin]);
    b = b(3:end);
    b = b(find([b.isdir]));
    %quality
    if (exist('quality', 'var') == 0) quality = 3; end
    ...
    pathm = [pathout 'mat/' pathin(ind(end-1)+1:end)];
    if (exist(pathm, 'dir') ~= 7) mkdir(pathm); end
    ...
    for ii = 1:numel(c)
        curSubDir = [c(ii).name '/'];
        a = dir([pathin curSubDir '*.tif']);
        n = size(a,1)
        disp([curSubDir]);
        directory = [pathin curSubDir]
        name = [pathin(ind(end-1):end-1) '_' curSubDir(1:end-1)]

        pivdir(directory, [pathm name], quality)
    end
    disp(['Saving ' pathm name '_done.txt'])
    done = 1;
    save([pathm name '_done.txt'], 'done', '-ascii');
    disp('All is save!');
end


function pivdir (directory, name, quality)
...
rb = 1;
%amount = min(length(filenames), 201*rb);
amount = min([length(filenames) ]);
num = amount;
% num = 100;
%quality
% if(sum(quality == (0:5)) == 0) quality = 2; end
quality
if(quality == -3)
...
if(quality == 1)
    ...
    s{8,1}= 'Int. area 3';           s{8,2}=64;         % third pass window size
    s{9,1}= 'Int. area 4';           s{9,2}=32;         % fourth pass window size
    s{10,1}='Win[name]dow deformation';    s{10,2}='*spline'; % '*spline' is more accurate, but slower
end
if(quality == 2)
    %% Standard PIV Settings
    s = cell(10,2); % To make it more readable, let's create a "settings table"
    ...
    s{9,1}= 'Int. area 4';           s{9,2}=24;         % fourth pass window size
    s{10,1}='Window deformation';    s{10,2}='*spline'; % '*spline' is more accurate, but slower
end
...
if(quality == 4)
    ...
    s = cell(10,2); % To make it more readable, let's create a "settings table"
    %Parameter                       %Setting           %Options
    s{1,1}= 'Int. area 1';           s{1,2}=96;         % window size of first pass
    s{2,1}= 'Step size 1';           s{2,2}=48;         % step of first pass
    s{3,1}= 'Subpix. finder';        s{3,2}=2;          % 1 = 3point Gauss, 2 = 2D Gauss
    s{4,1}= 'Mask';                  s{4,2}=[];         % If needed, generate via: imagesc(image); [temp,Mask{1,1},Mask{1,2}]=roipoly;
    s{5,1}= 'ROI';                   s{5,2}=[];         % Region of interest: [x,y,width,height] in pixels, may be left empty
    s{6,1}= 'Nr. of passes';         s{6,2}=4;          % 1-4 nr. of passes
    ...
    s{9,1}= 'Int. area 4';           s{9,2}=12;         % fourth pass window size
    s{10,1}='Window deformation';    s{10,2}='*spline'; % '*spline' is more accurate, but slower
end
if(quality == 5)
    %% Standard PIV Settings
...
rb = 1;
list=[];
for ii=1:rb:(amount-1)
    counter=counter+1;
    if((size(x{counter}, 1) == 0)&&(countNum<num))
        countNum = countNum + 1;
        list(countNum) = counter;
        list(countNum+1) = counter+1;
    end
end
x_temp = cell(countNum,1);
y_temp = x_temp;
u_temp = x_temp;
v_temp = x_temp;
tv_temp = x_temp;
...
%     if((size(x{counter}, 1) == 0)&&(countNum<num))
%     if((countNum<num))
    disp(['Counter ' num2str(counter) ' from ' num2str(amount-1)]);

    disp(['1: ' fullfile(directory, filenames{list(ii)})]);
    disp(['2: ' fullfile(directory, filenames{list(ii+1)})]);
%         countNum = countNum + 1;
    image1 = imread(fullfile(directory, filenames{list(ii)})); % read images
    image2 = imread(fullfile(directory, filenames{list(ii+1)}));
...
%     image2 = image2(1:2:end,1:2:end);
    ...
    [x_temp{ii} y_temp{ii} u_temp{ii} v_temp{ii} tv_temp{ii}] = piv_FFTmulti_s_GPU (image1,image2,s{1,2},s{2,2},s{3,2},s{4,2},s{5,2},s{6,2},s{7,2},s{8,2},s{9,2},s{10,2});
    if (ii==10) || mod(ii, 100) == 0
        for jj=1:countNum
            x{list(jj)} = x_temp{jj};
            y{list(jj)} = y_temp{jj};
            u{list(jj)} = u_temp{jj};
            v{list(jj)} = v_temp{jj};
            tv{list(jj)} = tv_temp{jj};
        end
...
% imdeform = s{10,2};
% 
% image1 = imread(image1name); % read images
% image2 = imread(image2name);
% image1 = image1(end:-1:1,:);
% image2 = image2(end:-1:1,:);
% 
% [x{1}, y{1}, u{1}, v{1}] = piv_FFTmulti (image1,image2,interrogationarea, step, subpixfinder, mask_inpt, roi_inpt, passes,int2,int3,int4,imdeform);
end

function [xtable ytable utable vtable typevector] = piv_FFTmulti_s_GPU (image1,image2,interrogationarea, step, subpixfinder, mask_inpt, roi_inpt, passes,int2,int3,int4,imdeform)
